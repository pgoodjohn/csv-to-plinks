...
pub fn command(command: &OpenCommand) -> Result<String, Box<dyn std::error::Error>> {
    ...
    rt.block_on(async {
        for record in &records {
            match get_payment_request_status(&command.api_key, &record.payment_link).await {
                Ok(payment_request_status) => match payment_request_status.paid_at {
                    Some(_t) => {}
                    None => {
                        log::info!(
                            "Payment Request ({} - EUR {}) for {} is still open: {}",
                            payment_request_status.id,
                            record.amount,
                            payment_request_status.description,
                            format!(
                                "https://paymentlink.mollie.com/payment/{}/",
                                &payment_request_status.id[3..]
                            )
                        );
                    }
                },
